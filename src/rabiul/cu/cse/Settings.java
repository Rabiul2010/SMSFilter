...
import bughunter2.smsfilter.R.drawable;
import bughunter2.smsfilter.R.string;
...
public class Settings extends SQLiteOpenHelper {
	...
	public static final String ACTION_NEW_MESSAGE = C.PACKAGE_NAME
			+ ".new_message";
	...
	public void onCreate(SQLiteDatabase db) {
		String sql = "CREATE TABLE " + SETTINGS_TABLE + "(" + KEY_ID
				+ " INTEGER PRIMARY KEY AUTOINCREMENT" + ", " + KEY_KEY
				+ " TEXT NOT NULL UNIQUE" + ", " + KEY_VALUE + " TEXT)";
		Log.d(TAG, "SQL:\n" + sql);
		db.execSQL(sql);

		sql = "CREATE TABLE " + FILTERS_TABLE + "(" + KEY_ID
				+ " INTEGER PRIMARY KEY AUTOINCREMENT" + ", " + KEY_NAME
				+ " TEXT NOT NULL UNIQUE" + ", " + KEY_ADDRESS
				+ " TEXT NOT NULL)";
		Log.d(TAG, "SQL:\n" + sql);
		db.execSQL(sql);

		sql = "CREATE TABLE " + CONTENT_STRINGS_TABLE + "(" + KEY_ID
				+ " INTEGER PRIMARY KEY AUTOINCREMENT" + ", " + KEY_FILTER_ID
				+ " INTEGER REFERENCES " + FILTERS_TABLE + "(" + KEY_ID
				+ ") ON DELETE CASCADE" + ", " + KEY_VALUE + " TEXT NOT NULL)";
		Log.d(TAG, "SQL:\n" + sql);
		db.execSQL(sql);

		sql = "CREATE TABLE " + MESSAGES_TABLE + "(" + KEY_ID
				+ " INTEGER PRIMARY KEY AUTOINCREMENT" + ", " + KEY_ADDRESS
				+ " TEXT NOT NULL" + ", " + KEY_RECEIVED_AT
				+ " INTEGER NOT NULL" + ", " + KEY_VALUE + " TEXT)";
		Log.d(TAG, "SQL:\n" + sql);
		db.execSQL(sql);
	}
	...
	public String getSetting(String key, String defaultValue) {
		...
		Cursor cursor = db.query(SETTINGS_TABLE, new String[] { KEY_VALUE },
				KEY_KEY + "=?", new String[] { key }, null, null, null);
	...
	public boolean saveMessages() {
		...
		return Boolean.parseBoolean(getSetting(SETTING_SAVE_MESSAGES,
	...
	public List<Filter> findFiltersByAddress(String address) {
		...
		SQLiteDatabase db = getReadableDatabase();
		Cursor cursor = db.query(FILTERS_TABLE, new String[] { KEY_NAME,
				KEY_ADDRESS }, KEY_ADDRESS + "=?", new String[] { address },
				null, null, FILTERS_ORDER_BY);
		while (cursor.moveToNext()) {
	...
	public List<Filter> getFilters() {
		...
		SQLiteDatabase db = getReadableDatabase();
		Cursor cursor = db.query(FILTERS_TABLE, new String[] { KEY_NAME,
				KEY_ADDRESS }, null, null, null, null, FILTERS_ORDER_BY);
		while (cursor.moveToNext()) {
			String name = cursor.getString(0);
			String address = cursor.getString(1);
			List<String> contentFilters = getContentFilters(name);
	...
	public List<String> getContentFilters(String filterName) {
		...
		SQLiteDatabase db = getReadableDatabase();
		String sql = "SELECT s." + KEY_VALUE + " FROM " + FILTERS_TABLE + " f"
				+ " JOIN " + CONTENT_STRINGS_TABLE + " s" + "  ON f." + KEY_ID
				+ "=s." + KEY_FILTER_ID + " WHERE f." + KEY_NAME + "=?";
	...
	public void deleteFilter(String name) {
		SQLiteDatabase db = getWritableDatabase();
		db.delete(FILTERS_TABLE, KEY_NAME + "=?", new String[] { name });
		db.close();
	}
	...
	public Message getMessage(long id) {
		...
		Cursor cursor = db.query(MESSAGES_TABLE, new String[] { KEY_ID,
				KEY_ADDRESS, KEY_RECEIVED_AT, KEY_VALUE }, KEY_ID + "=?",
	...
	public List<Message> getMessages() {
		...
		SQLiteDatabase db = getReadableDatabase();
		Cursor cursor = db.query(MESSAGES_TABLE, new String[] { KEY_ID,
				KEY_ADDRESS, KEY_RECEIVED_AT, KEY_VALUE }, null, null, null,
				null, MESSAGES_ORDER_BY);
		while (cursor.moveToNext())
	...
	private void showMessageNotification(long messageID) {
		Notification notification = Notifier.build(R.drawable.ic_stat_alert,
		...
		PendingIntent pendingIntent = PendingIntent.getActivity(mContext,
				MessageViewer.REQUEST_CODE_MUTATED, messageViewerIntent,
				PendingIntent.FLAG_ONE_SHOT);
		notification.setLatestEventInfo(mContext,
				mContext.getString(R.string.smsFilterBlockedAMessage),
				mContext.getString(R.string.clickToViewBlockedMessage),
				pendingIntent);
		Notifier.notify(mContext, String.valueOf(messageID),
				Notifier.NEW_MESSAGE, notification);
	}
}
