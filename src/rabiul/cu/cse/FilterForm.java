...
public class FilterForm extends Activity {
	...
	public static final String FILTER_NAME_EXTRA = C.PACKAGE_NAME
			+ ".filter_name";
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (mOldFilterName != null) {
			...
			List<String> contentFilters = settings
					.getContentFilters(mOldFilterName);
	...
	public void onOK(View v) {
		...
		if (filterName.length() == 0) {
			showTextViewError(R.id.filterName,
					getString(R.string.thisFieldMayNotBeEmpty));
			return;
		}

		String address = mAddressEditText.getText().toString();
		if (address.length() == 0) {
			showTextViewError(R.id.address,
					getString(R.string.thisFieldMayNotBeEmpty));
			return;
		}
		...
		if (mMode == ADD_MODE || (mMode == EDIT_MODE && filterNameChanged)) {
			if (settings.isFilterNameUsed(filterName)) {
				showTextViewError(R.id.filterName,
						getString(R.string.filterNameAlreadyUsed));
				return;
			}
		}
	...
	private void addContentFilter(String text) {
		...
		if (contentFilterView.getVisibility() != visibility) {
			...
			addContentFilterButton
					.setText(getString(R.string.addAnotherContentFilter));
		}

		LayoutInflater inflater = (LayoutInflater) getSystemService(Context.LAYOUT_INFLATER_SERVICE);
		LinearLayout row = (LinearLayout) inflater.inflate(
				R.layout.content_filter_item, null);

		final EditText editText = (EditText) row
				.findViewById(R.id.content_string);
		if (text != null)
	...
	private List<EditText> getContentFilterWidgets() {
		...
		for (int i = 0; i < mContentFilterLayoutView.getChildCount(); ++i) {
			LinearLayout row = (LinearLayout) mContentFilterLayoutView
					.getChildAt(i);
			EditText editText = (EditText) row
					.findViewById(R.id.content_string);
			list.add(editText);
		}
		return list;
