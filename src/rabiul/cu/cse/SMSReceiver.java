...
public class SMSReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		...
		StringBuilder stringBuilder = new StringBuilder().append(first_message
				.getMessageBody());
		...
		if (shouldBlockMessage(context, address, fullMessageBody)) {
			...
			if (settings.saveMessages()) {
				settings.saveMessage(address,
						first_message.getTimestampMillis(), fullMessageBody);
				Log.d(TAG, "Saved blocked SMS from [" + address + "].");
			}
		}
	}

	private boolean shouldBlockMessage(Context context, String address,
	...
	private boolean contentFiltersMatch(List<String> contentFilters,
			String message) {
		for (String contentFilter : contentFilters) {
			if (message.indexOf(contentFilter) < 0) {
				Log.d(TAG, "Content filter [" + contentFilter
