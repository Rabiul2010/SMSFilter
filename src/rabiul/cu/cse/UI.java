...
import bughunter2.smsfilter.R.id;
...
public class UI extends ListActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		views.add(SimpleListItem2.build(this, R.id.filters,
				getString(R.string.messageFilters),
				getString(R.string.viewAndEditMessageFilters)));

		views.add(SimpleListItem2.build(this, R.id.messages,
				getString(R.string.messages),
				getString(R.string.viewBlockedMessages)));

		CheckableLinearLayout checkableLinearLayout = CheckableLinearLayout
				.build(this, R.id.saveBlockedMessages,
		...
		if (settings.saveMessages())
			listView.setItemChecked(mSaveBlockedMessagesCheckableItemPosition,
					true);
	}
	...
	public void onListItemClick(ListView l, View v, int position, long id) {
		...
		if (itemID == R.id.filters) {
			...
			SparseBooleanArray checkedItemPositions = l
					.getCheckedItemPositions();
			boolean isChecked = checkedItemPositions
					.get(mSaveBlockedMessagesCheckableItemPosition);
	 ...
	 * won't work on newer versions either). Since Android 4.4, calling
	 * abortBroadcast() in our SMSReceiver.java no longer has the desired effect
	 * (namely preventing the default SMS application from receiving an SMS
	 * message that we wanted to block). See the following links for more
	 * information: https://code.google.com/p/android/issues/detail?id=61684
	 * https
	 * ://android-developers.blogspot.com/2013/10/getting-your-sms-apps-ready
	 * -for-kitkat.html
	 */
	private void showAppMayNotWorkWarning() {
		SpannableString message = new SpannableString(
				getString(R.string.appMayNotWork));
		Linkify.addLinks(message, Linkify.WEB_URLS);

		AlertDialog.Builder builder = new AlertDialog.Builder(this);
		builder.setIcon(android.R.drawable.ic_dialog_alert)
				.setTitle(R.string.sorry).setMessage(message)
